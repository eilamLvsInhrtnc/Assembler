...
int countWordsForData(char *line){
    ...
    char *label = strchr(pOL, ':'); // If label exists, skip it
    if (label) pOL = trimLeading(label + 1);

    if (strncmp(pOL, ".data", 5) == 0 && (isspace(pOL[5]) || pOL[5] == '\0')) {  // Handle .data directive
        ...
        for (int i = 0; params[i]; i++) {
            if (params[i] == ',' && params[i + 1] == ',') {
                fprintf(stderr, "Error: empty value in .data\n");
                return 0;
            }
        }   
    ...
    else if (strncmp(pOL, ".string", 7) == 0 && (isspace(pOL[7]) || pOL[7] == '\0')) { // Handle .string directive
    ...
    else if (strncmp(pOL, ".mat", 4) == 0 && (isspace(pOL[4]) || pOL[4] == '\0')) {  // Handle .mat directive
        char *args = trimLeading(pOL + 4);

        // parse rows
        char *open1 = strchr(args, '[');
        char *close1 = strchr(args, ']');
        if (!open1 || !close1 || close1 < open1) {
            fprintf(stderr, "Error: .mat invalid row brackets\n");
            return 0;
        }
        // parse columns
        *close1 = '\0';
        ...
        if (!open2 || !close2 || close2 < open2) {
            fprintf(stderr, "Error: .mat invalid column brackets\n");
            return 0;
        }
        *close2 = '\0';
        int cols = atoi(open2 + 1);
        if (rows <= 0 || cols <= 0) {
            fprintf(stderr, "Error: .mat dimensions must be positive integers\n");
            return 0;
        }
        int expectedCount = rows * cols;
        int actualCount = 0;
        char *valuesStart = trimLeading(close2 + 1); // Parse and validate matrix values
        if (*valuesStart) {
            char *token = strtok(valuesStart, ", \t");
            while (token) {
                token = trimLeading(token);
                if (!*token) { // Check for empty matrix value
                    fprintf(stderr, "Error: empty matrix value\n");
                    return 0;
                }
                ...
                actualCount++;
                if (actualCount > expectedCount) {  // Check for too many values
                    fprintf(stderr, "Error: too many matrix values — expected %d values\n", expectedCount);
                    return 0;
               }
                token = strtok(NULL, ", \t");
            }
